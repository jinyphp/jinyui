class ZSelect extends HTMLElement {
	...
	constructor() {
		...
		// 부속 요소 생성
		this._button = document.createElement('button'); // 확장버튼
		this._input = document.createElement('input'); // 값
		this._list = document.createElement('ul'); // 목록
	...
	init() {
		...
		if (this.hasAttribute('data-options')) {
			...
			this.removeAttribute('data-options'); //속성값 제거
		}
	...
	addOption({value, label, extra, class_name, is_disabled}, container, template) {
		...
		console.log(template || this._option_template)
		console.log({label: label.trim()})
		
		li.innerHTML = label.trim();
		/*
