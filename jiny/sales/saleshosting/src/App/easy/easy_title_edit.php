<?
	...
	$javascript = "<script>
		function popup_submit(mode){
            var url = \"/easy/easy_title_edit.php?popup_mode=\" + mode;
	...
	*/
    	...
    	if($popup_mode == "edit"){
			...
			if($rows = _mysqli_query_rows($query)){
				...
				*/
				...
				// 로고 파일 업로드 : CURL
				// if(!is_dir("./images")) mkdir("./images"); // 업로드 임시 폴더 확인
			
				if($files = _html_form_uploadfile("userfile1","index_title-$code") ) {
					// 로고 파일 업로드 성공시, query문 추가					
					// $query .= "`images`='index_title-$code.".$files[ext]."'";	
					$query->update("images","index_title-$code.".$files[ext]);
					rename("index_title-$code.".$files[ext], "../images/index_title-$code.".$files[ext]);
				}
		...
		$title = "EASY 타이틀 plug 이미지 설정";
		$body = $javascript._popup_body( $title, $width, _theme_popup($site_env->theme,"easy_title_edit",$site_language,$site_mobile) );
		...
		$query = "select * from site_index_title where code ='$plug[1]';";
		//echo $query."<br>";
		if($rows = _mysqli_query_rows($query)){
