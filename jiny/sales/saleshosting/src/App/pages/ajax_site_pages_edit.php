<?
	...
	$javascript = "<script>
		...
		function form_submit(mode,uid){
			...
			if(code){
				...
				alert(\"페이지 코드값을 입력해 주세요.\");
				$('#input_code').focus().css({'background-color':'#e6b9e9'});
			}		
		...
		// 이미지를 등록합니다.
		function image_upload(mode,code){
			var code = $('#input_code').val();
			if(code){
				...
				var upload = $('#input_upload').val();
				if(upload){

    				var parts = upload.split('.');
    				var ext = parts[parts.length - 1];
					// alert(ext);
					if(ext != \"jpg\" && ext != \"png\" &&  ext != \"gif\" &&  ext != \"bmp\"){ //확장자를 확인합니다.
						alert(\"이미지 파일(jpg, png, gif, bmp)만 업로드 가능합니다.\");
						return;
					}

					

					var url = \"ajax_site_pages_files.php?code=\"+code+\"&mode=\"+mode;
					var formData = new FormData($('#data')[0]);
					$.ajax({
						url:url,
        				type: 'POST',
					...
					$('#input_upload').focus().css({'background-color':'#e6b9e9'});
				}
		...
		function file_remove(mode,img){
			var returnValue = confirm(\"삭제하겠습니까?\");
			if(returnValue == true){
				var url = 'ajax_site_pages_files.php?mode=remove&img='+img;
	...
	if(isset($_SESSION['ajaxkey']) == $ajaxkey) { // Ajax CallKey Securities Checking...
		...
		/////////////
		$body = $javascript._theme_page($site_env->theme,"site_pages_edit",$site_language,$site_mobile);
