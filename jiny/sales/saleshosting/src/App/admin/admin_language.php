<?php
	...
	if(isset($_COOKIE['cookie_email'])){
		...
		$skin_name = "default";
		$body = _skin_body("default","site_language");
		...
		$body .= "<script>
				function language_mode(mode,uid){
					var url = \"/ajax_admin_language_editup.php?uid=\"+uid+\"&mode=\"+mode;
					...
					$.ajax({
                        ...
                        success:function(data){
                            $('#language_edit').html(data);
                        }
                    });
				}

                  function language_edit(mode,uid){
                  	var url = \"/ajax_admin_language_edit.php?uid=\"+uid+\"&mode=\"+mode;
		...
		$button ="<input type='button' value='NEW' onclick=\"javascript:language_edit('new','0')\" id=\"".$btn_style_gray."\" >";          
		$body = str_replace("{new}",$button,$body);

		// 카테고리 리스트는
        // ajax 형태로 처리함.
		$_SESSION['ajaxkey'] = $ajaxkey = md5('ajaxkey'.$_SERVER['PHP_SELF'].$TODAYTIME.microtime()); 	
		$body = str_replace("{language_list}","
					<form name='cate' method='post' enctype='multipart/form-data' action='".$_SERVER['PHP_SELF']."'>
					   <input type='hidden' name='ajaxkey' value='$ajaxkey'>
					 
					<span id=\"language_list\">
		...
		$body = str_replace("{edit}","<span id=\"language_edit\"></span>",$body);
