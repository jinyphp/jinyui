		...
		if($header_rows = _mysqli_query_rows($query)){
			...
			if($rowss1 = _mysqli_query_rowss($query1)){
				...
				for($i=0;$i<count($rowss1);$i++){
					...
					$query1 = "UPDATE `site_skin_html` SET  `html` = '$mobile' where `code`='$footer' and `eid`='$uid' and `language`='".$rows1->code."' and `mobile`='mobile'";
					_mysqli_query($query1);
				}
			...
			if($rowss1 = _mysqli_query_rowss($query1)){
				...
				for($i=0;$i<count($rowss1);$i++){
					...
					$mobile =  addslashes( _formdata($rows1->code."_m") );
					$query1 = "INSERT INTO `site_skin_html` (`eid`,`language`,`mobile`,`html`,`code`,`domain`) 
					VALUES ('".$uid."','".$rows1->code."','mobile','$mobile','footer','".$site_env_rows->domain."')";
					_mysqli_query($query1);
				}
