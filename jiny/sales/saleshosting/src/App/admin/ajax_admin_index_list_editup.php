		...
		if($mode == "delete"){
			...
			if($rowss1 = _mysqli_query_rowss($query1)){
				...
				for($i=0;$i<count($rowss1);$i++){
					...
					if(_mysqli_query_rows($query)){
						$desktop_html =  addslashes( _formdata($rows1->code) );
						$query1 ="UPDATE `site_skin_html` SET `html`='$desktop_html' where `code`='index' and `eid`='$eid' and `language`='".$rows1->code."' and `mobile`='pc' ";
					...
					if(_mysqli_query_rows($query)){
		    			...
		    			$query1 ="UPDATE `site_skin_html` SET `html`='$mobile_html' where `code`='index' and `eid`='$eid' and `language`='".$rows1->code."' and `mobile`='m' ";
		    			_mysqli_query($query1);
		    			echo $query1."<br>";
					} else {
						$mobile_html =  addslashes( _formdata($rows1->code."_m") );
