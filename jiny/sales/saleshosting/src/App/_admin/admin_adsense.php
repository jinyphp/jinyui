<?
	...
	if($_COOKIE[adminemail]){ ///////////////	
		...
		$body = admin_shopskin("admin_adsense");
		
		$body = str_replace("{new}",skin_button("추가","admin_adsensenew.php"),$body);
		...
		if(mysql_affected_rows()) {
			...
			for($i=0;$i<$total;$i++){
				...
				if($MOBILE == "mobile") {
					...
					$list .= "<table border='0' cellpadding='5' cellspacing='5' width='100%'><tr>";
					$list .= "<td width='50' bgcolor='ffffff'> <font size=2> {<a href='admin_adsenseedit.php?UID=$rows[Id]'>$rows[code]</a>}</font></td>";
