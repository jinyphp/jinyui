	...
	if($_COOKIE[adminemail]){ ///////////////
    	...
    	switch($mode){
    		case 'del':
    			if($_GET['UID']){
    				$UID = $_GET['UID'];
    				$query = "DELETE FROM `shop_pages` WHERE `Id`='$UID'";
    				echo $query."<br>";
					...
					$mobile = $_GET['mobile'];
    				$query = "DELETE FROM `shop_pages` WHERE `code`='$code' and mobile ='$mobile' and language = '$language'";
    				echo $query."<br>";
    				mysql_db_query($mysql_dbname,$query,$connect);
				}
    			// page_back2();
    		...
    		case 'editup':
    			...
    			if(!$code) msg_alert("오류! 페이지 이름이 없습니다.");
    			else {
    				$result=mysql_db_query($mysql_dbname,"select * from `shop_pages` where code = '$code' and language = '$skin_language' and mobile='$skin_mobile'",$connect);
					if( mysql_affected_rows() ){ 
		    			...
		    			$query ="UPDATE `shop_pages` SET `domain`='$domain', `language`='$skin_language', `enable`='$enable', 
		    			...
		    			$query = "INSERT INTO `shop_pages` (`regdate`, `domain`, `enable`, `code`, `layout`, `html`, `mobile`, `language`) 
    					...
    					if ($_FILES["userfile1"][tmp_name]){
   		 					...
   		 					if(!is_dir("../pages")) $an = mkdir("../pages");
  				
							$uploadfile1  = "../pages/".$_FILES[userfile1][name];
							...
							if ($_FILES["userfile1"][tmp_name]) $filename1 = "../pages/$code--1";
			
    	  					if ($_FILES["userfile".$i][tmp_name]) {
    	     					move_uploaded_file($_FILES["userfile".$i][tmp_name], "../pages/$code--1.".$ext);
    	  					}
    	  						
    	  					$query = "UPDATE `shop_pages` SET `images1`='./pages/$code--1.".$ext."'  WHERE `code`='$code' and `regdate`='$regdate'";
    						// echo $query."<br>";
    						mysql_db_query($mysql_dbname,$query,$connect); 
    	  						
							$images1 = "./pages/$code--1.".$ext;
    					...
    					if ($_FILES["userfile2"][tmp_name]){
							...
							$uploadfile2  = "../pages/".$_FILES[userfile2][name];
							...
							if ($_FILES["userfile2"][tmp_name]) $filename2 = "../pages/$code--2";
			
    	  					if ($_FILES["userfile".$i][tmp_name]) {
    	     					move_uploaded_file($_FILES["userfile".$i][tmp_name], "../pages/$code--2.".$ext);
    	  					}
    	  						
    	  					$query = "UPDATE `shop_pages` SET `images2`='./pages/$code--2.".$ext."'  WHERE `code`='$code' and `regdate`='$regdate'";
    						// echo $query."<br>";
    						mysql_db_query($mysql_dbname,$query,$connect); 
    	  						
							$images2 = "./pages/$code--2.".$ext;
    		...
    		default:
				...
				$body = admin_shopskin("admin_pages_new");
				
				// $body = str_replace("./sales_script.js","../sales_script.js",$body);
				
				if($_GET['code']){
					$code = $_GET['code'];
    				$query = "select * from `shop_pages` where code = '$code' and language ='".$_SESSION['language']."' and mobile ='".$_SESSION['mobile']."'";
    			} else if($_GET['UID']){
    				$UID = $_GET['UID'];
    				$query = "select * from `shop_pages` where Id = '$UID'";
    			}
    			
    			$result=mysql_db_query($mysql_dbname,"$query",$connect);
				if( mysql_affected_rows() ) $rows=mysql_fetch_array($result);
    			
    			$body=str_replace("{#formstart}","<form name='skinbody' method='post' enctype='multipart/form-data' action='admin_pages_new.php'> 
				...
				$body = str_replace("{#skinbody}","<textarea name='html' rows='20' style='width:100%'>$html</textarea>
													",$body);
					
		
			$body = str_replace("{#submit}","<input type='submit' name='reg' value='수정' $css_submit >",$body);						
			$body = str_replace("{#delete}","<input type='button' value='삭제' onclick='pageClick(\"admin_pages_new.php?mode=del&UID=$UID&code=$code&language=".$_SESSION['language']."&mobile=".$_SESSION['mobile']."\")' $css_submit>",$body);
