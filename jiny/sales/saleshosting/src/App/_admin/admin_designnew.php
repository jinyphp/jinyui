<?
	...
	if($_COOKIE[adminemail]){ ///////////////
    			...
    			$body=str_replace("{formstart}","<form name='country1' method='post' enctype='multipart/form-data' action='admin_designnewup.php'> 
					    				<input type='hidden' name='nonce' value='$nonce'>
					    				<input type='hidden' name='mode' value='newup'>",$body);
				$body = str_replace("{formend}","</form>",$body);
					

				$body = str_replace("{pages}","<input type='text' name='pages' $cssFormStyle >",$body);		
				$body = str_replace("{pc_skin}","<textarea name='pc_skin' rows='8' style='width:100%'></textarea>",$body);
				$body = str_replace("{mobile_skin}","<textarea name='mobile_skin' rows='8' style='width:100%'></textarea>",$body);
