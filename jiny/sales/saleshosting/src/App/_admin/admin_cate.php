<?
	...
	if($_COOKIE[adminemail]){ ///////////////	
		...
		switch($mode){
			case 'up': 
				...
				if( mysql_affected_rows() ){ 
		    		...
		    		$query1 = "select * from `shop_cate` where level = '$rows[level]' and pos < $rows[pos] and ref = $rows[ref] order by pos desc "; 
		    		//echo $query1."<br>";;
					$result1 = mysql_db_query($mysql_dbname,$query1,$connect);
					if( mysql_affected_rows() ){ 
		    			$rows1 = mysql_fetch_array($result1); 
		    			
		    			$query2 = "select * from `shop_cate` where tree like '%$UID%' order by pos desc";
		    			//echo $query2."<br>";
						...
						if( mysql_affected_rows() ){ 
		    				...
		    				//***
		    				$query3 = "select * from `shop_cate` where tree like '%$UID%' order by pos desc";
		    				//echo $query3."<br>";
		    				...
		    				//***
		    				//***
		    				$query3 = "select * from `shop_cate` where tree like '%$rows1[Id]%' order by pos desc";
		    				//echo $query3."<br>";
		    				$result3=mysql_db_query($mysql_dbname,str_replace("*","count(*)",$query3),$connect);
    						$total3=mysql_result($result3,0,0);
			...
			case 'down': 
				...
				if( mysql_affected_rows() ){ 
		    		...
		    		$query1 = "select * from `shop_cate` where level = '$rows[level]' and pos > $rows[pos] and ref = $rows[ref] order by pos asc "; 
		    		//echo $query1."<br>";
					$result1 = mysql_db_query($mysql_dbname,$query1,$connect);
					if( mysql_affected_rows() ){ 
		    			$rows1 = mysql_fetch_array($result1); 
		    			...
		    			$query2 = "select * from `shop_cate` where tree like '%$rows1[Id]%' order by pos desc";
		    			//echo $query2."<br>";
						$result2 = mysql_db_query($mysql_dbname,$query2,$connect);
						if( mysql_affected_rows() ){ 
		    				$rows2 = mysql_fetch_array($result2);
			...
			case 'disable':
				if($_SESSION['nonce'] == $_GET['nonce']){
					$_SESSION['nonce'] = $nonce = md5('goodname'.microtime());
					$query = "UPDATE `shop_cate` SET `enable` = '' WHERE `tree` like '%$UID%'";
			...
			case 'enable':
				if($_SESSION['nonce'] == $_GET['nonce']){
					$_SESSION['nonce'] = $nonce = md5('goodname'.microtime());
					$query = "UPDATE `shop_cate` SET `enable` = 'on' WHERE `tree` like '%$UID%'";
					mysql_db_query($mysql_dbname,$query,$connect);
				} else msg_alert("오류! 중복실행."); 
				break;
		...
		$body = admin_shopskin("admin_cate");
		
		$body = str_replace("{new}",skin_button("추가","admin_catenew.php"),$body);    
		...
		if(mysql_affected_rows()) {
			...
			for($i=0;$i<$total;$i++){
				...
				if($MOBILE == "mobile") {
					...
					$list .= "<td bgcolor='ffffff'> <font size=2> <a href='admin_catenew.php?UID=$rows[Id]&pos=$rows[pos]&level=$rows[level]'>+</a> <a href='admin_cate.php?mode=up&UID=$rows[Id]'>▲</a><a href='admin_cate.php?mode=down&UID=$rows[Id]'>▼</a> $LevelSpace <a href='admin_cateedit.php?UID=$rows[Id]'>$rows[catename]</a> </font></td>";
					...
					if($rows[enable]) $list .= "<td width='40' bgcolor='ffffff'> <font size=2>$rows[Id] <a href='admin_cate.php?mode=disable&UID=$rows[Id]&nonce=$nonce'>▣</a></font></td>";
					else $list .= "<td width='40' bgcolor='ffffff'> <font size=2>$rows[Id] <a href='admin_cate.php?mode=enable&UID=$rows[Id]&nonce=$nonce'>□</a></font></td>";
					...
					$list .= "<td bgcolor='ffffff'> <font size=2> $depth <a href='admin_catenew.php?UID=$rows[Id]&pos=$rows[pos]&level=$rows[level]'>+</a> <a href='admin_cate.php?mode=up&UID=$rows[Id]&nonce=$nonce'>▲</a><a href='admin_cate.php?mode=down&UID=$rows[Id]&nonce=$nonce'>▼</a>  <a href='admin_cateedit.php?UID=$rows[Id]'>$rows[catename]</a> </font></td>";
