<?
	...
	if($_COOKIE[adminemail]){ ///////////////	
		...
		$body = admin_shopskin("admin_currency"); 
		
		$body = str_replace("{new}",skin_button("통화추가","admin_currency_new.php"),$body); 
		...
		if(mysql_affected_rows()) {
			...
			for($i=0;$i<$total;$i++){
				...
				$listform = admin_bodyskin("admin_currency_list","pc","ko");
				
				if($rows[enable]) $listform = str_replace("{en}","▣",$listform);
				else $listform = str_replace("{en}","□",$listform);	
				
				$listform = str_replace("{currency}","<a href='admin_currencyedit.php?UID=$rows[Id]'>$rows[currency]</a>",$listform);
