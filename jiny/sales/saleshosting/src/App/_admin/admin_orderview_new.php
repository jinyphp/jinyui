<?
	...
	if($_COOKIE[adminemail]){ ///////////////
		...
		switch($mode){
			...
			case 'order_ok':
				$cartlog = $_GET['cartlog'];					
    			
    			$query = "UPDATE `shop_order` SET `status`='ordering' WHERE `cartlog`='$cartlog'";
    			mysql_db_query($mysql_dbname,$query,$connect);
    							
        		echo "<meta http-equiv='refresh' content='0; url=admin_orders.php?status=ordering'>";
				break;

					
			default:
				//# 화면 디자인 템플릿을 스킨 읽어옵니다. 
    			$body = admin_shopskin("admin_orderview_new");
    			...
    			*/
 				...
 				$cartlog = $_GET['cartlog'];
				$body = str_replace("{order_ok}",skin_button("입금확인","admin_orderview_new.php?mode=order_ok&cartlog=$cartlog"),$body);  		
    			$body = str_replace("{print}",skin_button("인쇄","#"),$body); 		
