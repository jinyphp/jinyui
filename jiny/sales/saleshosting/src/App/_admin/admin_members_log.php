<?
	...
	if($_COOKIE[adminemail]){ ///////////////
		...
		$body = admin_shopskin("admin_members_log");
		...
		if($_count <= 0){
			...
			$count = $total; 
			$query .= " order by lastlog desc LIMIT 0 , $total";
			echo "카운트조건 1 $_count <= 0 ... ".$_GET['count']." <br>";
		} else {
			if($limit) $query .= " order by lastlog desc LIMIT $limit , $_count"; else $query .= " order by lastlog desc LIMIT 0 , $_count";
		...
		if(mysql_affected_rows()) {
			for($i=0;$i<$count;$i++){
				$rows=mysql_fetch_array($result);
					$listfrom = admin_bodyskin("admin_members_log_list","pc","ko");
					
					$listfrom = str_replace("{regdate}","$rows[regdate]",$listfrom);
					
					$listfrom = str_replace("{name}","$rows[username]",$listfrom);
					$listfrom = str_replace("{email}","$rows[email]",$listfrom);
