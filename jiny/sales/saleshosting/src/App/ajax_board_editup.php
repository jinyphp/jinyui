<?php
	...
	if(isset($_SESSION['ajaxkey']) == $ajaxkey) { // Ajax CallKey Securities Checking...
		...
		if($mode == "new"){
			...
			$board_rows = _mysqli_query_rows($query); 
    			...
    			if( $rows = _mysqli_query_rows($query) ){
					...
					for($i=0,$j=1;$i<count($files_label);$i++,$j++){
						...
						if($_FILES[$userfile]['tmp_name']){
							...
							if($files = _form_uploadfile($userfile,"./board/$board/".$rows->Id."/".$_FILES[$userfile]['name'])) {
			...
			if($rows = _mysqli_query_rows($query)){
				...
				for($i=0,$j=1;$i<count($files_label);$i++,$j++){
					...
					if($_FILES[$userfile]['tmp_name']){
						...
						if($files = _form_uploadfile($userfile,"./board/$board/".$uid."/".$_FILES[$userfile]['name'])) {
							// 파일 업록드
			...
			if($rows = _mysqli_query_rows($query)){
    			...
    			// 계시판 파일자료 삭제
				exec("rm -rf ./board/$board/$uid",$output);   	
		...
		// AJAX : 페이지 목록 이동 
		$url = "board.php"."?limit=$limit&searchkey=$search&board=".$board;    		
