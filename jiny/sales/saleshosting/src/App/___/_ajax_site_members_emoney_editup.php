	...
	if(isset($_SESSION['ajaxkey']) == _formdata("ajaxkey")) { // Ajax CallKey Securities Checking...
		...
		if($mode == "delete"){
			...
			if($rows = _sales_query_rows($query)){
    			...
    			$balance = $rows->balance - $rows->emoney;

    			// Balance 값 조정
				$query = "select * from `site_members_emoney` where Id>'$uid' and email = '".$members_rows->email."'";
				if($rowss1 = _sales_query_rowss($query)){
					for($i=0;$i<count($rowss1);$i++){
						$rows1 = $rowss1[$i];
						$balance = $balance + $rows1->emoney;
						$query = "UPDATE `site_members_emoney` SET `balance`='$balance' WHERE `Id`='".$rows1->Id."'";
						//echo $query."<br>";
						_sales_query($query);
					}
				}

				$query = "UPDATE `site_members` SET emoney='$balance' where email='".$members_rows->email."'";
				//echo $query."<br>";
				_sales_query($query);
			...
			if($rows = _sales_query_rows($query)){
				...
				$query = "UPDATE `site_members_emoney` SET ";
				...
				$balance = $members_rows->emoney + intval($emoney);
				$query = "UPDATE `site_members` SET emoney='$balance' where email='".$members_rows->email."'";
				//echo $query."<br>";
				_sales_query($query);
		...
		echo "<script>
				$.ajax({
            		url:'/ajax_site_members_emoney.php?uid=".$mem."&ajaxkey=".$ajaxkey."',
            		type:'post',
            		data:$('form').serialize(),
            		success:function(data){
            			$('.mainbody').html(data);
