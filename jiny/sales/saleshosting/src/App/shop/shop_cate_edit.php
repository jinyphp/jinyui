<?
	...
	if(isset($_COOKIE['cookie_email'])){
		...
		// $javascript = "<script>"._javascript_ajax_html("#mainbody","/ajax_site_menu_edit.php?ajaxkey=$ajaxkey&uid=$uid&mode=$mode&menu_code=$menu_code")."</script>";
		$javascript = "<form name='menu' method='post' enctype='multipart/form-data' action='".$_SERVER['PHP_SELF']."'>
						<input type='hidden' name='ajaxkey' value='$ajaxkey'>
						<input type='hidden' name='uid' value='$uid'>
						<input type='hidden' name='mode' value='$mode'>
						<input type='hidden' name='menu_code' value='$menu_code'>";
		$javascript .= "<script>"._javascript_ajax_html("#mainbody","ajax_shop_cate_edit.php")."</script>";
		$javascript .= "</form>";
