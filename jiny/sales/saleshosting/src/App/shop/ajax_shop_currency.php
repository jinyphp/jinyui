<?
	...
	$javascript = "<script>
		function mode(mode,uid,limit){
			var url = \"ajax_shop_currency_editup.php\";
		...
		function edit(mode,uid,limit){
            var url = \"shop_currency_edit.php\";		
			var form = document.shop;
			form.action = url;  //이동할 페이지
  			form.mode.value = mode;
  			form.uid.value = uid;
        ...
        function list(limit){
			var url = \"ajax_shop_currency.php\";
	...
	if(isset($_SESSION['ajaxkey']) == $ajaxkey) { // Ajax CallKey Securities Checking...
		...
		$body = $javascript._theme_page($site_env->theme,"shop_currency",$site_language,$site_mobile);
		...
		///////////////////
		// 주문목록 및 이력 출력  
		$query = "select * from `shop_currency` order by regdate desc";
		if($rowss = _sales_query_rowss($query)){	
			...
			$list .= "<td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;' >이름</td>";
			$list .= "</tr></table>";
			...
			for($i=0,$total_prices=0,$sub_prices=0; $i<count($rowss); $i++){
				...
				if($rows->enable) $list .= "<td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;' width='20' id=\"table_td\"> 
						<a href='#' onclick=\"javascript:mode('disable','".$rows->Id."')\"> <i class=\"fa fa-check-square-o\"></i> </a></td>";
				else $list .= "<td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;' width='20' id=\"table_td\"> 
						<a href='#' onclick=\"javascript:mode('enable','".$rows->Id."')\"> <i class=\"fa fa-square-o\"></i> </a></td>";

				$list .= "<td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;' width='50'>".$rows->currency."</td>";
				
				$list .= "<td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;'><a href='#' onclick=\"javascript:edit('edit','".$rows->Id."')\" >".$rows->name."</a></td>";
				$list .= "</tr></table>";
