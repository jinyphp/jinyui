<?
	...
	$javascript = "<script>
		function mode(mode,uid,limit){
			var url = \"ajax_shop_country_editup.php\";
	...
	if(isset($_SESSION['ajaxkey']) == $ajaxkey) { // Ajax CallKey Securities Checking...
		...
		$body = $javascript._theme_page($site_env->theme,"shop_country",$site_language,$site_mobile);
		...
		if($rowss = _sales_query_rowss($query)){	
			...
			$list .= "<td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;' width='50'>국가코드</td>";
			$list .= "<td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;' width='100'>치환코드</td>";
			$list .= "<td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;' >스트링</td>";
			$list .= "</tr></table>";
			...
			for($i=0,$total_prices=0,$sub_prices=0; $i<count($rowss); $i++){
				...
				if($rows->enable) $list .= "<td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;' width='20' id=\"table_td\"> 
						<a href='#' onclick=\"javascript:country_mode('disable','".$rows->Id."')\"> <i class=\"fa fa-check-square-o\"></i> </a></td>";
				else $list .= "<td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;' width='20' id=\"table_td\"> 
						<a href='#' onclick=\"javascript:country_mode('enable','".$rows->Id."')\"> <i class=\"fa fa-square-o\"></i> </a></td>";
