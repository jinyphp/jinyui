<?
	...
	if(!isset($_COOKIE[Session]) && !isset($_COOKIE[email]) ) login();    
		...
		if( mysql_affected_rows() ){ 
			...
			//# 스킨 레이아웃 
			$body = shopskin("sales_report_sell");
    		...
    		$body = str_replace("{print}","$script <input type='button' value='인쇄' $btn_style_green onclick=\"javascript:pageprint()\">",$body);	
    		...
    		if( mysql_num_rows($result) ){ 	
				...
				for($i=0;$i<$total;$i++){
	    			...
	    			$listform = bodyskin("sales_report_sell_subtitle",$_SESSION['mobile'],$_SESSION['language']);
    				...
    				if( mysql_num_rows($result1) ){ 				
						...
						for($j=0;$j<$total1;$j++){
	    					...
	    					$listform = bodyskin("sales_report_sell_list",$_SESSION['mobile'],$_SESSION['language']);
