	...
	if(!isset($_COOKIE[Session]) && !isset($_COOKIE[email]) ) {
		...
		if(  mysql_num_rows($result)  ){ 
			...
			if($mode == "send"){
										...
										if(  mysql_num_rows($result)  ){ 
											...
											if(  mysql_num_rows($result)  ){
												...
												$query = "INSERT INTO `sales_trans_$mem1[Id]` (`regdate`, `transdate`, `memid`, `trans`,  `UIDA`, `UIDB`,  
														...
														VALUES ('$TODAYTIME', '$transdate', '$MEM[Id]', 'sell',  '$COMA', '$COMB',  
				...
				$body = shopskin("sales_trans_buy_expert");
    			...
    			$body = str_replace("{send}","$script <input type='submit' value='전송' $btn_style_blue >",$body);	
				...
				if($TID){
					for($i=0,$amount=0;$i<count($TID);$i++){
    					...
    					if( mysql_num_rows($result) ) {
    						...
    						$listform = bodyskin("sales_trans_buy_expert_list",$_SESSION['mobile'],$_SESSION['language']);
