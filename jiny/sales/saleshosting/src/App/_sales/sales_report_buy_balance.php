<?
	...
	if(!isset($_COOKIE[Session]) && !isset($_COOKIE[email]) ) login();    
		...
		if( mysql_affected_rows() ){ 
			...
			$body = str_replace("{title}","<h1 align='center'>매출처 미지급</h1>",$body);
    		...
    		if( mysql_num_rows($result) ){ 	
				...
				for($i=0;$i<$total;$i++){
					...
					//마지막 입금일 지연 경과
					$query1 = "SELECT * FROM `sales_trans_$MEM[Id]` where trans = 'buy_payin' and UIDB = '$rows[Id]' order by transdate desc, Id desc";
					//echo $query1."<br>";
					$result1=mysql_db_query($server[dbname],$query1,$dbconnect);
    				if( mysql_num_rows($result1) ){
    					$trans=mysql_fetch_array($result1); 
