<?
	...
	if(!isset($_COOKIE[Session]) && !isset($_COOKIE[email]) ) {
		...
		if(  mysql_num_rows($result)  ){ 
			...
			$body = shopskin("sales_quotation");
			$body = str_replace("{new}",_button_black("신규작성","sales_quotation_new.php?mode=new"),$body);
			...
			$body = str_replace("{search}","<input type='text' name='search' $cssFormStyle placeholder='견적명'>",$body);	
			...
			$query = "select * from `sales_quotation_$MEM[Id]` where type = 'quotation' ";
    		...
    		if( mysql_num_rows($result) ){ 
	    		...
	    		if($_POST['goods']) $listform = str_replace("{nodata}","검색된 견적이 없습니다.",$listform);
				else $listform = str_replace("{nodata}","등록된 견적이 없습니다.",$listform);
