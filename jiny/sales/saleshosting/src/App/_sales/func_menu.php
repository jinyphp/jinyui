<?
	...
	function _menu_tree(){
		...
		$query = "select * from `site_menu` where code='$code' and enable = 'on' order by pos asc";
		if($rowss = _sales_query_rowss($query)){
			...
			for($i=0;$i<count($rowss);$i++){
				...
				if($rows = _sales_query_rows($query)) $menubar .= "<li class='has-sub'>"; else $menubar .= "<li>";
				...
				if($rows = _sales_query_rows($query)){
					...
					if($rows = _sales_query_rows($query)) $menubar .= "</li>\n";	else $menubar .= "\n</ul>\n </li>\n";
				}
	...
	function _menu_setting($code){
		global $master_dbconnect, $master_mysql;

		$query = "select * from `site_menu_setting` where code='$code'";	
		$result=mysql_db_query($master_mysql[dbname],$query,$master_dbconnect);
	...
	function _menu($code){
		...
		if( $total ) {
			...
			for($i=0;$i<$total;$i++){
				...
				if(  mysql_num_rows($result1)  ) $menubar .= "<li class='has-sub'>"; else $menubar .= "<li>";
				...
				if(  mysql_num_rows($result1)  )	{
					...
					$result1=mysql_db_query($master_mysql[dbname],$query1,$master_dbconnect);
					if( mysql_num_rows($result1) ) 	$menubar .= "</li>\n";	else $menubar .= "\n</ul>\n </li>\n";
				}
