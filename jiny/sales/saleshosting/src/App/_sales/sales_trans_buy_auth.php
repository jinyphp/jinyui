	...
	if(!isset($_COOKIE[Session]) && !isset($_COOKIE[email]) ) {
		...
		if(  mysql_num_rows($result)  ){ 
			...
			if($mode == "auth"){
				...
				for($i=0;$i<count($TID);$i++){
    				...
    				if( mysql_num_rows($result) ) {
    					...
    					if( mysql_num_rows($result1) ) {
    						...
    						$balance2 = $rows1[balance2] + $rows[prices_total]; // Balance2 매입
				...
				echo "<meta http-equiv='refresh' content='0; url=sales_trans_buy_auth.php'>";
			} else {
			
			
			
				$body = shopskin("sales_trans_buy_auth");
				...
				//# 전표 자료 출력
				$query = "select * from sales_trans_$MEM[Id] where trans = 'buy' and memid != $MEM[Id] and auth IS NULL"; //판매 전표출력    	
    			...
    			if( mysql_num_rows($result) ){ 				
					for($i=0;$i<$total;$i++){
	    				...
	    				$listform = bodyskin("sales_trans_buy_auth_list",$_SESSION['mobile'],$_SESSION['language']);
