<?
	...
	if(isset($_SESSION['ajaxkey']) == $ajaxkey) { // Ajax CallKey Securities Checking...
		...
		$category = _formdata("category");	echo "category = $category <br>";
		...
		if($mode == "enable"){
			...
			if($rows = _sales_query_rows($query)){
				...
				if($language_rows = _sales_query_rowss($query1)){
			...
			//$query->insert("sales_countrys",_formdata("sales_countrys"));
			...
			echo _curl_post("http://".$sales_db->domain."/goods/curl_goods_editup.php",$_POST);
