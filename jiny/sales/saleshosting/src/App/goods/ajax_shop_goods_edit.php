<?
	...
	$javascript = "<script>
		...
		function image_remove(mode,gid,images){
			var url = 'ajax_shop_goods_images.php?gid='+gid+'&mode='+mode+'&images='+images;
	...
	if(isset($_SESSION['ajaxkey']) == $ajaxkey) { // Ajax CallKey Securities Checking...
		...
		$body = str_replace("{attach_label}",_form_text("attach_label",$goods->attach_label,$css_textbox),$body);
		...
		for($k=1;$k<10;$k++){
			...
			if($goods->$img) {
					...
					$img_url = "http://".$sales_db->domain."/".$goods->$img;
					$images_body .="<div style=\"font-size:12px;padding:10px;\">
