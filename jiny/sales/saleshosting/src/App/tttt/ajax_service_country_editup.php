<?
	...
	if(isset($_SESSION['ajaxkey']) == isset($_POST['ajaxkey'])) { // Ajax CallKey Securities Checking...
		...
		function _country_enable($uid){
			$query = "UPDATE `country` SET `enable` = '' WHERE `Id` like '$uid'";
			_sales_query($query);
		...
		function _country_disable($uid){
			$query = "UPDATE `country` SET `enable` = 'on' WHERE `Id` like '$uid'";
			_sales_query($query);
		...
		function _country_delete($uid){
			$query = "DELETE FROM `country` WHERE `Id`='$uid'";
			_sales_query($query);
		...
		if($mode == "edit"){
			...
			if($flag == false ){
				...
				$query = "UPDATE `country` SET  `language`='$language', `currency`='$currency', `tax`='$tax',
			...
			if($flag == false ){
    			...
    			$query = "INSERT INTO `country` (`address`, `phone`, `fax`, `email`,`language`, `currency`, `tax`,`name`, `code`, `replace_code`, `enable`) 
