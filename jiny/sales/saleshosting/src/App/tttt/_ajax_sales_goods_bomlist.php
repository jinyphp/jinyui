	...
	if(isset($_SESSION['ajaxkey']) == _formdata("ajaxkey")) { // Ajax CallKey Securities Checking...
		...
		if($rowss = _sales_query_rowss($query)){
			...
			for($i=0; $i<count($rowss); $i++){
				...
				$list .= "<table border=\"0\" width=\"100%\" cellspacing=\"0\" cellpadding=\"0\"><tr>";
				$list .= "<td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;'><a href='#' onclick=\"javascript:goods_select('".$rows->Id."','".$rows->name."','".$rows->prices_sell."')\">".$rows->name."</ㅁ></td>";
