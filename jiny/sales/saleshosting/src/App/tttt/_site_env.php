	...
	if(isset($_COOKIE['cookie_email'])){ // 로그인 접속 확인
		...
		$body = str_replace("<!--{skin_emptybody}-->","
					...
					<form name='goods' method='post' enctype='multipart/form-data'>
						...
						<script>"._javascript_ajax_html("#mainbody","ajax_site_env.php")."</script>				
					</form>",$body);
		echo $body;
		...
		$javascript .= "<script>"._javascript_ajax_html("#mainbody","/ajax_site_env.php")."</script>";			
		$javascript .= "</form>";
