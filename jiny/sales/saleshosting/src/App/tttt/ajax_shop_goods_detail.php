	...
	if(isset($_SESSION['ajaxkey']) == isset($_POST['ajaxkey'])) { // Ajax CallKey Securities Checking...
		...
		// echo $mode;
			...
			if($goods = _shop_goods_rows($uid)){
				...
				$body = str_replace("{trans_goods}","<input type='button' value='상품연동' onclick=\"javascript:form_submit('trans','".$uid."')\" style=\"".$css_btn_gray."\" >",$body);
				$body = str_replace("{relation}","<input type='button' value='관련상품' onclick=\"javascript:form_submit('relation','".$uid."')\" style=\"".$css_btn_gray."\" >",$body);
				$body = str_replace("{resales}","<input type='button' value='상품입점' onclick=\"javascript:resales('resales','".$uid."')\" style=\"".$css_btn_gray."\" >",$body);
			} else {
			// echo "상품 정보를 읽어 올수 없습니다.";
				...
				$body = str_replace("{trans_goods}","<input type='button' value='상품연동' style=\"".$css_btn_gray."\" >",$body);
				$body = str_replace("{relation}","<input type='button' value='관련상품' style=\"".$css_btn_gray."\" >",$body);
				$body = str_replace("{resales}","<input type='button' value='상품입점' style=\"".$css_btn_gray."\" >",$body);
			...
			*/
			...
			if($goods->images2) {
				// $img_url = $goods->images2;
				$img_url = "http://".$sales_db->domain."/".$goods->images2;
				$body = str_replace("{goodimg2}","<img src='".$img_url."' border='0' width='300'>",$body);
			...
			if($goods->images3) {
				// $img_url = $goods->images3;
				$img_url = "http://".$sales_db->domain."/".$goods->images3;
				$body = str_replace("{goodimg3}","<img src='".$img_url."' border='0' width='300'>",$body);
