<?
	...
	if(isset($_COOKIE['cookie_email'])){
		...
		/*
		$skin_name = "default";
		$body = _skin_body("default","shop_shipping");
		...
		$button ="<input type='button' value='NEW' onclick=\"javascript:shipping_edit('new','0')\" id=\"".$btn_style_gray."\" >";          
		$body = str_replace("{new}",$button,$body);


		// Form and Ajax Process
		$body = str_replace("{shipping_list}","
					<span id=\"shipping_list\">

					<script>
						$.ajax({
            				url:'/ajax_shop_shipping.php',
            				type:'post',
            				data:$('form').serialize(),
            				success:function(data){
		...
		$body = str_replace("{edit}","<span id=\"shipping_edit\"></span>",$body);
		...
		*/
		...
		// Form and Ajax Process
		$_SESSION['ajaxkey'] = $ajaxkey = md5('ajaxkey'.$_SERVER['PHP_SELF'].$TODAYTIME.microtime()); 	
		$body = str_replace("<!--{skin_emptybody}-->","<script>"._javascript_ajax_html("#mainbody","/ajax_shop_shipping.php?ajaxkey=$ajaxkey")."</script>",$body);
