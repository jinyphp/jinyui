	...
	$script = "<script>
		...
		function form_submit(mode,uid){
        	...
        	if(program){
        		...
        		if(code){
        			var url = \"/ajax_hosting_users_editup.php?uid=\"+uid+\"&mode=\"+mode;
					$.ajax({
               			url:url,
                		type:'post',
                		data:$('form').serialize(),
	...
	if(isset($_SESSION['ajaxkey']) == $ajaxkey) { // Ajax CallKey Securities Checking...
		...
		// 가입 되지 않은 회원만 처리
			...
			if($rows = _mysqli_query_rows($query)){
				...
				$body = $script._skin_page($skin_name,"hosting_service_new");
				...
				// 파트너 프로그램 표시 
				$query = "select * from `service_products` where reseller = '".$members->reseller."' order by level asc";
				if($rowss = _mysqli_query_rowss($query)){	
					...
					// echo $list;
					$body = str_replace("{hosting_plan}","<div id=\"partner_program\">".$list."</div>",$body);
