<?
	...
	if(isset($_SESSION['ajaxkey']) == isset($_POST['ajaxkey'])) { // Ajax CallKey Securities Checking...
		...
		// 지정된 상품 하나를 읽어옴
		function _site_layout_rows($uid){
			$query = "select * from `site_layout` WHERE `Id`='$uid'";
			//echo $query;
		...
		/////////////
		$body = _skin_page($skin_name,"site_layout_edit");
		...
		$body = str_replace("{formend}","</form>",$body);
			...
			if($layout_rows = _site_layout_rows($uid)){
