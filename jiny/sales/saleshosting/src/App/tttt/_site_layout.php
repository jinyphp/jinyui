<?
	...
	if(isset($_COOKIE['cookie_email'])){
		...
		$skin_name = "default";
		$body = _skin_body("default","site_layout");
		...
		$body .= "<script>
				...
				function block_edit(mode,uid){
                  	$.ajax({
                        url:'/ajax_site_layout_edit.php?uid='+uid+'&mode='+mode,
                        type:'post',
                        data:$('form').serialize(),
		...
		$body = str_replace("{layout_list}","
					<form name='layout' method='post' enctype='multipart/form-data' action='".$_SERVER['PHP_SELF']."'>
					   <input type='hidden' name='ajaxkey' value='$ajaxkey'>
					<span id=\"layout_list\">
					
					<script>
						$.ajax({
            				url:'/ajax_site_layout.php',
            				type:'post',
            				data:$('form').serialize(),
            				success:function(data){
            					$('.mainbody').html(data);
