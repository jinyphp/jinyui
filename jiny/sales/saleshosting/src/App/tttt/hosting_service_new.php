<?
	...
	if(isset($_COOKIE['cookie_email'])){ // 로그인 접속 확인
		...
		if($reseller = _hosting_rows($email)){
		//이미 가입된 회원
			$msg = "가입된 회원입니다.";
			$javascript = "<script>"._javascript_ajax_html("#mainbody","/ajax_service_info.php?ajaxkey=$ajaxkey")."</script>";
		...
		// 가입 되지 않은 회원만 처리
			...
			if($rows = _mysqli_query_rows($query)){
				$msg = "이미 신청된 회원 입니다.";
				$javascript = "<script> alert(\"$msg\"); "._javascript_ajax_html("#mainbody","/ajax_hosting_renewal.php?ajaxkey=$ajaxkey")."</script>";	
			} else {
				$javascript = "<script>"._javascript_ajax_html("#mainbody","/ajax_hosting_service_new.php?ajaxkey=$ajaxkey")."</script>";	
			}
