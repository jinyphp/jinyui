<?
	...
	if(isset($_SESSION['ajaxkey']) == isset($_POST['ajaxkey'])) { // Ajax CallKey Securities Checking...
		...
		$skin_name = "default";
		$body = _skin_page("default","site_header_edit");
		...
		if($uid){
			...
			$script = "<script>
				...
				function form_submit(mode,uid){
					var url = \"/ajax_admin_header_editup.php?uid=\"+uid+\"&mode=\"+mode;
			...
			if($rowss1 = _mysqli_query_rowss($query1)){
				...
				for($i=0;$i<count($rowss1);$i++){
					...
					$code = $rows1->code;	
					$desktop = _site_skin_html("header",$uid,$code,"pc");
					$desktop_html = stripslashes($desktop->html);

					$mobile = _site_skin_html("header",$uid,$code,"mobile");
