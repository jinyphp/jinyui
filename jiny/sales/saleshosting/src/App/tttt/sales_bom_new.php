<?
	...
	if(!isset($_COOKIE[Session]) && !isset($_COOKIE[email]) ) {
		...
		if( mysql_num_rows($result) ){ 
    		...
    		$body = shopskin("sales_bom_new");
    		include "sales_bom_left.php";
    			
			/////////////////////////////

			$body=str_replace("{formstart}","<form name='good' method='post' enctype='multipart/form-data' action='sales_bom_newup.php'> 
			...
			// $body = str_replace("{company}","<input type='text' name='company1' value='$COM[company]' $cssFormStyle placeholder='공급거래처'>",$body);
