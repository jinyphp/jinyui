	...
	if(isset($_COOKIE['cookie_email'])){
		...
		$script = "
				...
				function index_edit(mode,uid){
                  	var url = \"/ajax_admin_index_list.php?uid=\"+uid+\"&mode=\"+mode;	
                  	$.ajax({
                        url:url,
                        type:'post',
                        data:$('form').serialize(),
				...
				function form_submit(mode,uid){
					var url = \"/ajax_admin_index_editup.php?mode=\"+mode+\"&uid=\"+uid;
				...
				function form_list_submit(mode,eid){
					var url = \"/ajax_admin_index_list_editup.php?eid=\"+eid+\"&mode=\"+mode;
					var formData = new FormData($('#data')[0]);
					alert(url);
					$.ajax({
						url:url,
				...
				function form_list_delete(mode,eid){
					var url = \"/ajax_admin_index_title_editup.php?eid=\"+eid+\"&mode=\"+mode;
				...
				function form_title_submit(mode,uid){
					var url = \"/ajax_admin_index_title_editup.php?uid=\"+uid+\"&mode=\"+mode;
					var formData = new FormData($('#data')[0]);
					alert(url);
					$.ajax({
						url:url,
				...
				function form_goods_submit(mode,uid){
					var url = \"/ajax_admin_index_goods_editup.php?uid=\"+uid+\"&mode=\"+mode;
					var formData = new FormData($('#data')[0]);
					alert(url);
					$.ajax({
						url:url,
