	...
	if(isset($_SESSION['ajaxkey']) == $ajaxkey) { // Ajax CallKey Securities Checking...
        ...
        if($mode == "edit"){
			...
			if($rows = _sales_query_rows($query)){	
				if($rows->type == "payin"){
					$body = $javascript._skin_page($skin_name,"site_members_emoney_payin");
					...
					$body = $javascript._skin_page($skin_name,"site_members_emoney_payout");
					$body = str_replace("{close}","<input type=\"button\" onclick=\"popup_close();\" value=\"Close\" style=\"".$css_btn_gray."\"/>",$body);
			...
			$body = str_replace("{email}","<input type='text' name='email' style=\"$css_textbox\">",$body);
