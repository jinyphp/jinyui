<?
	...
	if(isset($_SESSION['ajaxkey']) == $ajaxkey) { // Ajax CallKey Securities Checking...
		...
		if($mode == "delete"){
    		...
    		$query = "DELETE FROM sales_manager_level WHERE `Id`='$uid'";
    		_sales_query($query);
			...
			if($rows = _sales_query_rows($query)){
				// 수정 
				// 이메일 중복여부 체크
				$query1 = "select * from sales_manager_level where email='$email'";
				if($rows->email != $email && $rows1 = _sales_query_rows($query1)){
					echo "중복된 담당자 입니다.";
				} else {
					$query = "UPDATE sales_manager_level SET ";
				...
				if($rows->email != $email && $rows1 = _sales_query_rows($query1)){
					...
					$query = "INSERT INTO sales_manager_level ($insert_filed) VALUES ($insert_value)";
		...
		$url = "sales_manager_level.php"."?limit=$limit&searchkey=$search&list_num=".$list_num;    		
