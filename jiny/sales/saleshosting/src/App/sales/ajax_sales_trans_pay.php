<?
	...
	if(isset($_SESSION['ajaxkey']) == $ajaxkey) { // Ajax CallKey Securities Checking...
		...
		$title = "전표 결제";
		$body = $javascript._popup_body( $title, $width, _theme_popup($site_env->theme,"sales_trans_pay",$site_language,$site_mobile) );

		$body = str_replace("{formstart}","<form name='popup' method='post' enctype='multipart/form-data'>
											<input type='hidden' name='mode' value='paynow'>
					   						<input type='hidden' name='ajaxkey' value='".$ajaxkey."'>",$body);
		$body = str_replace("{formend}","</form>",$body);


		$body = str_replace("{form_submit}","
				<input type='button' value='결제' onclick=\"javascript:popup_submit()\" style=\"".$css_btn_gray."\" >
				",$body);
		...
		if($TID){
			$list = "<table border='0' width='100%' cellspacing='0' cellpadding='0' style='background-color:#DEDEDF;border-top:1px solid #D2D2D2;border-right:1px solid #D2D2D2;border-bottom:1px solid #D2D2D2;border-left:1px solid #D2D2D2'>
					<tr>
					<td style='border-right:1px solid #E9E9E9;font-size:12px;padding:5px;' width=\"10\"></td>
			...
			for($i=0,$amount=0;$i<count($TID);$i++){
    			...
    			if($rows = _sales_query_rows($query)){	
					...
					<tr>
					<td style='border-right:1px solid #E9E9E9;font-size:12px;padding:5px;' width=\"10\"> <input type='checkbox' name='TID1[]' value='$TID[$i]' checked>  </td>
