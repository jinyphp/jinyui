<?
	...
	$javascript = "<script>
		function mode(mode,uid,limit){
			var url = \"ajax_sales_recruit_editup.php\";
		...
		function edit(mode,uid,limit){
            var url = \"sales_recruit_edit.php\";		
			var form = document.sales;
			form.action = url;  //이동할 페이지
  			form.mode.value = mode;
  			form.uid.value = uid;
        ...
        function list(limit){
			var url = \"ajax_sales_recruit.php\";
	...
	if(isset($_SESSION['ajaxkey']) == $ajaxkey) { // Ajax CallKey Securities Checking...
		...
		$body = $javascript._theme_page($site_env->theme,"sales_recruit",$site_language,$site_mobile);
		...
		///////////////////
		// 상품 목록을 검색
		$query = "select * from `sales_recruit` ";
		if($searchkey) {
		...
		if($rowss = _sales_query_rowss($query)){
			...
			$list .= "<td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;' width='120'>등록일자</td>
						<td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;' >채용 공고명</td>
						<td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;' width='120'>만기일</td>
							</tr>
						</table>";
			...
			for($i=0;$i<$count;$i++){
				...
				$list .= "<td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;' width='120'>".$rows->regdate."</td>
							...
							<td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;' width='120'>".$rows->expire."</td>
							</tr>
						</table>";				
