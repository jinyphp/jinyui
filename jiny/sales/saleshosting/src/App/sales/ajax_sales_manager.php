<?
	...
	$javascript = "<script>
		function mode(mode,uid,limit){
			var url = \"ajax_sales_manager_editup.php\";
		...
		function edit(mode,uid,limit){
            var url = \"sales_manager_edit.php\";		
			var form = document.sales;
			form.action = url;  //이동할 페이지
  			form.mode.value = mode;
  			form.uid.value = uid;
        ...
        function list(limit){
			var url = \"ajax_sales_manager.php\";
	...
	if(isset($_SESSION['ajaxkey']) == $ajaxkey) { // Ajax CallKey Securities Checking...
		...
		$body = $javascript._theme_page($site_env->theme,"sales_manager",$site_language,$site_mobile);
		...
		///////////////////
		// 상품 목록을 검색
		$query = "select * from `sales_manager` ";
		if($searchkey) {
		...
		if($rowss = _sales_query_rowss($query)){
			...
			$list .= "<td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;' width='100'>담당자</td>
					  ...
					  <td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;' width='100'>부서명</td>
					  <td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;' width='100'>직급</td>
					  <td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;' width='150'>연락처</td>
							</tr>
						</table>";
			...
			for($i=0;$i<$count;$i++){
				...
				$title_name = "<a href='#' onclick=\"javascript:edit('edit','".$rows->Id."','".$limit."')\">".$rows->firstname." ".$rows->lastname."</a>";
				...
				$list .= "<td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;' width='100'>".$title_name."</td>
						  <td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;'>".$rows->email."</td>
						  <td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;' width='100'>".$rows->part."</td>
						  <td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;' width='100'>".$rows->level."</td>
						  <td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;' width='150'>".$rows->phone."</td>
							</tr>
						</table>";				
