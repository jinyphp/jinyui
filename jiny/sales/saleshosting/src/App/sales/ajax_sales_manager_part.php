<?
	...
	$javascript = "<script>
		function mode(mode,uid,limit){
			var url = \"ajax_sales_manager_part_editup.php\";
		...
		function edit(mode,uid,limit){
            var url = \"sales_manager_part_edit.php\";		
			var form = document.sales;
			form.action = url;  //이동할 페이지
  			form.mode.value = mode;
  			form.uid.value = uid;
        ...
        function list(limit){
			var url = \"ajax_sales_manager_part.php\";
	...
	if(isset($_SESSION['ajaxkey']) == $ajaxkey) { // Ajax CallKey Securities Checking...
		...
		$body = $javascript._theme_page($site_env->theme,"sales_manager_part",$site_language,$site_mobile);
		...
		///////////////////
		// 상품 목록을 검색
		$query = "select * from `sales_manager_part` ";
		if($searchkey) {
		...
		if($rowss = _sales_query_rowss($query)){
			...
			$list .= "<td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;' >부서명</td>
							</tr>
						</table>";
