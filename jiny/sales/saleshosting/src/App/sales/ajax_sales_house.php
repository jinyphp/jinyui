<?
	...
	$javascript = "<script>
		function mode(mode,uid,limit){
			var url = \"ajax_sales_house_editup.php\";
	...
	if(isset($_SESSION['ajaxkey']) == $ajaxkey) { // Ajax CallKey Securities Checking...
		...
		$body = $javascript._theme_page($site_env->theme,"sales_house",$site_language,$site_mobile);
		...
		$button ="<input type='button' value='제품추가' onclick=\"javascript:edit('new','0','0')\" id=\"css_btn_new\">";          
		...
		if($rowss = _sales_query_rowss($query)){
			...
			$list .= "<td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;' width='100'>국가</td>
					  <td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;'>창고명</td>
			...
			for($i=0;$i<$count;$i++){
				...
				if($rows->manager){
					...
					// echo $query1;
					$manager_rows = _sales_query_rows($query1);

					$manager = $manager_rows->firstname." ".$manager_rows->lastname." (".$manager_rows->email.")";
				} else $manager = "";

				if($rows->business){
					$query1 = "select * from sales_business where Id ='".$rows->business."'";
					// echo $query1;
					$business_rows = _sales_query_rows($query1);

					$business = $business_rows->business." (".$business_rows->country.")";
				} else $business = "";
				...
				$list .= "<td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;' width='100'>".$rows->country."</td>
