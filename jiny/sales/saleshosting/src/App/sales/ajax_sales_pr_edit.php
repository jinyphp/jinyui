<?
	...
	$javascript = "<script>
		function form_submit(mode,uid){
			var url = \"ajax_sales_pr_editup.php?uid=\"+uid+\"&mode=\"+mode;
	...
	if(isset($_SESSION['ajaxkey']) == $ajaxkey) { // Ajax CallKey Securities Checking...
		...
		$body = $javascript._theme_page($site_env->theme,"sales_pr_edit",$site_language,$site_mobile);
		...
		$query = "select * from sales_pr where Id = '$uid'";
		// echo $query."<br>";
		...
		$body = str_replace("{title}","<input type='text' name='title'  value='".$rows->title."' style=\"$css_textbox\">",$body);
		// $body = str_replace("{expire}","<input type='date' name='expire'  value='".$rows->expire."' style=\"$css_textbox\">",$body);
		/*
		...
		*/
		$body = str_replace("{content}","<textarea name='content' rows='10' style='$css_textarea'>".stripslashes($rows->content)."</textarea>",$body);
