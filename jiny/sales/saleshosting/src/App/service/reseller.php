<?
	...
	if(isset($_COOKIE['cookie_email'])){
		...
		if($reseller = _service_resellerRows($email)){
			//리셀러 회원일 경우, 리셀러 목록을 출력합니다.
			$javascript = "<script>"._javascript_ajax_html("#mainbody","ajax_reseller.php")."</script>";


		} else {
			// 리셀러 회원이 아닌경우, 리셀러 회원가입 화면으로 이동
			// 회원가입전, 가입신청 이력이 있는지 검색
			$query = "select * from service.reseller_renewal where email ='$email'";
			if($rows = _mysqli_query_rows($query)){
				$msg = "이미 리셀러 신청된 회원 입니다.";
				$javascript = "<script> 
					alert(\"$msg\"); 
					"._javascript_ajax_html("#mainbody","ajax_reseller_renewal.php")."
					</script>";
						
			} else {
				$javascript = "<script>"._javascript_ajax_html("#mainbody","ajax_reseller_regist.php")."</script>";	
			}
		}		
