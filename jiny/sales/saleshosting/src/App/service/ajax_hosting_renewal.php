<?
	...
	$javascript = "<script>
		...
		function edit(mode,uid){
			var url = \"ajax_hosting_renewal_view.php?mode=\"+mode+\"&uid=\"+uid;
			popup_ajax(url);
        }
	...
	if(isset($_SESSION['ajaxkey']) == $ajaxkey) { // Ajax CallKey Securities Checking...
		...
		$button ="<input type='button' value='삭제' onclick=\"javascript:delete('delete','0')\" id=\"css_btn_gray\">";          
		$body = str_replace("{delete}",$button,$body);

		$button ="<input type='button' value='승인' onclick=\"javascript:auth('auth','0')\" id=\"css_btn_gray\">";          
		$body = str_replace("{auth}",$button,$body);

		$button ="<input type='button' value='승인취소' onclick=\"javascript:disauth('disauth','0')\" id=\"css_btn_gray\">";          
		...
		if($reseller = _is_reseller($email)){
			// 리셀러 : 하부 리셀러 관리
			$query = "select * from service.service_host_renewal where reseller = '$email' ";
		} else {
			// 미승인, 미가입 처리된 회원 = 자기 정보만 출력
			$query = "select * from service.service_host_renewal where email = '$email' ";
		}
		
		if($mode) $query .= " and type = '$mode' ";
		...
		if($rowss = _mysqli_query_rowss($query)){
			...
			$list = "<table border=\"0\" width=\"100%\" cellspacing=\"0\" cellpadding=\"0\">
							...
							<td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;' width='100'>구분</td>
							<td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;' width='100'>신청자</td>
							...
							<td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;' width='80'>결제금액</td>
			...
			for($i=0;$i<$count;$i++){
				...
				// if($rows->type == "regist") $renewalType = "신규신청"; else if($rows->type == "renewal") $renewalType = "연장";
				$renewalType = $rows->type;
				...
				$list .= "<table border=\"0\" width=\"100%\" cellspacing=\"0\" cellpadding=\"0\">
							...
							<td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;' width='100'>".$renewalType."</td>
							<td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;' width='100'>".$users->username."</td>
