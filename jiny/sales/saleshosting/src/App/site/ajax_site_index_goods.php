<?
	...
	$javascript = "<script>
		...
		function edit(mode,uid){
			var url = \"site_index_goods_edit.php\";
        ...
        function list(limit){
            var url = \"ajax_site_index_goods.php?limit=\"+limit;
	...
	if( isset($_SESSION['ajaxkey']) == $ajaxkey ) { // Ajax CallKey Securities Checking...
		...
		$body = $javascript._theme_page($site_env->theme,"site_plug_goods",$site_language,$site_mobile);
		...
		$query = "select * from `site_index_goods` order by regdate desc ";
		...
		if($rowss = _sales_query_rowss($query)){	
			...
			$list .= "<table border='0' width='100%' cellspacing='0' cellpadding='0'><tr>
				<td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;' width=150>전처리코드</td>
				<td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;'>타이틀</td>
				<td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;' width=100>연결 카테고리</td>
				</tr></table>";
			...
			for($i=0; $i<$count; $i++){
				...
				$list .= "<a href='#' onclick=\"javascript:edit('edit','".$rows->Id."')\" >{goodlist_".$rows->code."}</a><br>";
				$list .= "</td>
				<td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;'>".$rows->title."</td>
				<td style='border-bottom:1px solid #E9E9E9;font-size:12px;padding:10px;' width=50>".$rows->cate."</td>
				</tr></table>";
			}
