<?
	...
	$javascript = "<script>
		...
		function form_submit(mode,uid){
			var url = \"ajax_site_cssfile_editup.php?mode=\"+mode+\"&uid=\"+uid;
	...
	if(isset($_SESSION['ajaxkey']) == $ajaxkey) { // Ajax CallKey Securities Checking...
		...
		// 지정된 상품 하나를 읽어옴
		function _site_cssfile_rows($uid){
			$query = "select * from `site_css_files` WHERE `Id`='$uid'";
			//echo $query;
		...
		$body = $javascript._theme_page($site_env->theme,"site_cssfile_edit",$site_language,$site_mobile);
		...
		$body = str_replace("{formend}","</form>",$body);
			...
			if($css_rows = _site_cssfile_rows($uid)){
			...
			$body = str_replace("{css_file}",$rows->css_file,$body);
