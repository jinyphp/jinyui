<?php
	...
	// ===========================================================
	// 페이지 로딩시, 국가 체크  
	$__site_country = _formdata("country");
	if( $__site_country && $__site_country != ""){
		// 직접 입력
		
		$site_country = $__site_country;
		$_SESSION['session_country'] = $__site_country;
		
	} else {
	
		if(isset($_SESSION['sesstion_country'])){
			//섹션값이 설정되어 있을 경우, 섹션값 적용.
			//echo "섹션값이 설정되어 있을 경우, 섹션값 적용.";
			$site_country = $_SESSION['sesstion_country'];
		} else {
			$domain = $_SERVER['HTTP_HOST'];
			$query = "select * from `site_env` where domain = '".$domain."'";
			//echo $query."<br>";
			if($rows = _mysqli_query_rows($query)){
				$site_country = $rows->country;
			} else {
				$site_country = "kr";
			}
		}
	...
	function _shop_country_rows(){
	// 활성화된 국가 리스트만 표기	
		$query = "select * from `shop_country` where enable = 'on'";
		if($rowss = _mysqli_query_rowss($query)){
			return $rowss;
		}
	}

	function _country_bar($country,$lang){
		...
		if($rowss = _mysqli_query_rowss($query)){
			...
			for($list="",$i=0;$i<count($rowss);$i++){
				...
				if($lang) $country_name = $name->$lang; else $country_name = $name->ko; 
				...
				*/
				$url = $_SERVER['PHP_SELF']."?country=".$rows->code;
				$list .= "<a href='$url'>".$country_name."</a>";
