<?
	...
	$javascript = "<script>
		...
		function mode(mode,uid,limit){
			var url = \"ajax_site_members_blacklist_editup.php\";
		...
		function edit(mode,uid,limit){
			var url = \"site_members_blacklist_edit.php\";
        ...
        function list(limit){
        	var url = \"ajax_site_blacklist_members.php\";
        	var form = document.site;
        	form.limit.value = limit;
	...
	if(isset($_SESSION['ajaxkey']) == $ajaxkey) { // Ajax CallKey Securities Checking...
		...
		$body = $javascript._theme_page($site_env->theme,"site_members_blacklist",$site_language,$site_mobile);
		...
		$query = "select * from `site_members_black` ";
		// echo $query."<br>";
		if($searchkey) {
		...
		if($rowss = _sales_query_rowss($query)){	
			...
			for($i=0;$i<$count;$i++){
				...
				$list .= "<a href='#' onclick=\"javascript:edit('edit','".$rows->Id."','$limit')\" >".$rows->email."</a>";
				$list .= "</td></tr></table>";
			}
