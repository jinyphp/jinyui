<?
	...
	if(isset($_SESSION['ajaxkey']) == $ajaxkey) { // Ajax CallKey Securities Checking...
		...
		if($mode == "disable"){
			$query = "UPDATE `site_members_black` SET `enable`='' WHERE `Id`='$uid'";
			_sales_query($query);	//echo $query."<br>";

		} else if($mode == "enable"){
			$query = "UPDATE `site_members_black` SET `enable`='on' WHERE `Id`='$uid'";
			_sales_query($query);	//echo $query."<br>";
		...
		$url = "site_members_blacklist.php"."?limit=$limit&searchkey=$search";    		
