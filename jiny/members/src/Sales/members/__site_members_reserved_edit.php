<?
	...
	if(isset($_COOKIE['cookie_email'])){
		...
		$body = str_replace("<!--{skin_emptybody}-->","
					...
					<form name='goods' method='post' enctype='multipart/form-data'>
					   ...
					   <input type='hidden' name='uid' value='$uid'>
						...
						<script>"._javascript_ajax_html("#mainbody","ajax_site_members_reserved_edit.php")."</script>				
					</form>",$body);

		
		
		/*
		$body = _skin_body($skin_name,"site_members_reserved");

		$script = "
				<script>
				function site_edit(mode,uid){
                  	var url = \"/ajax_site_members_reserved_edit.php?uid=\"+uid+\"&mode=\"+mode;	
                  	$.ajax({
                        url:url,
                        type:'post',
                        data:$('form').serialize(),
				...
				function form_submit(mode,uid){
					var url = \"/ajax_site_members_reserved_editup.php?mode=\"+mode+\"&uid=\"+uid;
		...
		$_SESSION['ajaxkey'] = $ajaxkey = md5('ajaxkey'.$_SERVER['PHP_SELF'].$TODAYTIME.microtime()); 
		$body=str_replace("{formstart}",$script."<form id='data' name='members' method='post' enctype='multipart/form-data' action='".$_SERVER['PHP_SELF']."'> 
		...
		// Form and Ajax Process
		$body = str_replace("{reserved_list}","
					<span id=\"site_list\">

					<script>
						$.ajax({
            				url:'/ajax_site_members_reserved.php',
            				type:'post',
            				data:$('form').serialize(),
            				success:function(data){
            					$('#site_list').html(data);
