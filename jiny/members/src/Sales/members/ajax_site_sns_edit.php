<?
	...
	$javascript = "<script>
		...
		function form_submit(mode,uid){
			var url = \"ajax_site_sns_editup.php?uid=\"+uid+\"&mode=\"+mode;
	...
	if(isset($_SESSION['ajaxkey']) == $ajaxkey) { // Ajax CallKey Securities Checking...
		...
		$body = $javascript._theme_page($site_env->theme,"site_sns_edit",$site_language,$site_mobile);
		...
		//////////////////
		if($uid){
			$query = "select * from `site_sns` where Id =$uid";	//echo $query."<br>";			
		...
		$body = str_replace("{client_id}",_form_text("client_id",$rows->client_id,$css_textbox),$body);
		$body = str_replace("{client_secret}",_form_text("client_secret",$rows->client_secret,$css_textbox),$body);
		$body = str_replace("{callback}",_form_text("callback",$rows->callback,$css_textbox),$body);
		$body = str_replace("{facebook}",_form_text("facebook",$rows->facebook,$css_textbox),$body);
