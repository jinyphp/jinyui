...
class CTabView extends CDiv {
	...
	public function __construct($data = []) {
		...
		$this->addClass(CSS_STYLE_TABLE_FORMS_CONTAINER);
	}
	...
	public function toString($destroy = true) {
		if (count($this->tabs) == 1) {
			...
			// 텝 메뉴 (ul 요소 생성)
			$headersList = (new CList())->addClass('ui-tabs-nav');
			...
			foreach ($this->headers as $id => $header) {
				...
				$li->setAttribute(":class", "{'ui-tabs-active' : tab === '".$id."'}" ); // AlpinJS

				// 기본 선택 
                if(isset($this->selectedTab) && $this->selectedTab == $id) {
					$li->addClass("ui-tabs-active"); // 보여지는 텝은 클래스 추가
                } 
